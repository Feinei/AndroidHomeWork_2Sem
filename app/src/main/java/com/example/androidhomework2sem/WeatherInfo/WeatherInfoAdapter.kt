...
class WeatherInfoAdapter(
    ...
    override fun areItemsTheSame(oldItem: WeatherInfo, newItem: WeatherInfo): Boolean = oldItem.name == newItem.name

    override fun areContentsTheSame(oldItem: WeatherInfo, newItem: WeatherInfo): Boolean = oldItem == newItem
}) {
    ...
    override fun onBindViewHolder(holder: WeatherInfoItemHolder, position: Int) = holder.bind(getItem(position))
}